...
STARTUP_NUM_FRAMES = 10  # Number of interpolation frames for startup sequence
STARTUP_FRAME_DELAY = 0.2  # Seconds between startup frames (5 * dt where dt=0.05)

# Startup target joint positions (CHEST1, ARM0-3_LEFT, ARM0-3_RIGHT)
# arm raised position
# STARTUP_CHEST1 = 0.0
...
STARTUP_CHEST1 = 0.0
STARTUP_ARM0_LEFT = 0.0
STARTUP_ARM1_LEFT = 0.0
STARTUP_ARM2_LEFT = 0.0
STARTUP_ARM3_LEFT = 0.0
STARTUP_ARM0_RIGHT = 0.0
STARTUP_ARM1_RIGHT = 0.0
STARTUP_ARM2_RIGHT = 0.0
STARTUP_ARM3_RIGHT = 0.0
...
class MobileIKController(Node):
    def __init__(self, use_ik=False):
        ...
        self.sdf = SignedDistanceField(
            bounds=ObstacleAvoidanceConfig.SDF_BOUNDS, 
            resolution=ObstacleAvoidanceConfig.SDF_RESOLUTION,
            max_distance=ObstacleAvoidanceConfig.SDF_MAX_DISTANCE,
        ...
        self.get_logger().info("Publishing startup interpolation sequence...")
        ...
        # Publish startup sequence before starting control loop
        if self.use_ik:
            self.get_logger().info("Using IK startup sequence (--use-ik enabled)")
            self.publish_startup_sequence_ik()
        else:
            self.get_logger().info("Using standard startup sequence")
            self.publish_startup_sequence()
    ...
    def pointcloud_callback(self, msg):
        ...
        try:
            ...
            range_mask = ((distances < ObstacleAvoidanceConfig.POINTCLOUD_MAX_RANGE) & 
                         (distances > ObstacleAvoidanceConfig.POINTCLOUD_MIN_RANGE))
            points = points[range_mask]
            
            # Downsample for efficiency
            if len(points) > ObstacleAvoidanceConfig.POINTCLOUD_DOWNSAMPLE_SIZE:
                indices = np.random.choice(len(points), ObstacleAvoidanceConfig.POINTCLOUD_DOWNSAMPLE_SIZE, replace=False)
            ...
            with self.obstacle_lock:
                ...
                if self.sdf_update_counter >= ObstacleAvoidanceConfig.SDF_UPDATE_RATE:
    ...
    def publish_startup_sequence(self):
        """Publish 5 interpolated frames from zero position to startup target"""
        try:
            self.get_logger().info(f"Starting smooth startup sequence ({STARTUP_NUM_FRAMES} frames)...")
            ...
            # Call service synchronously during startup
            ...
            init_position = np.zeros(len(self.startup_target))
            
            for i in range(STARTUP_NUM_FRAMES):
                # Linear interpolation from 0 to target
                alpha = (i + 1) / STARTUP_NUM_FRAMES
                interpolated_position = init_position + alpha * self.startup_target
                ...
                self.get_logger().info(f"Published startup frame {i+1}/{STARTUP_NUM_FRAMES} (alpha={alpha:.2f})")
                
                # Wait for dt between frames
                time.sleep(STARTUP_FRAME_DELAY)
            
            self.startup_complete = True
            self.get_logger().info("Startup sequence complete. Beginning normal control loop.")
            
        except Exception as e:
            self.get_logger().error(f"Error during startup sequence: {e}")
            self.startup_complete = True  # Continue anyway

    def publish_startup_sequence_ik(self):
        """Publish startup sequence for ia_robot_ik with virtual joints (no interpolation)"""
        try:
            self.get_logger().info("Publishing ia_robot_ik startup sequence...")
            ...
            # Target positions for upper body (same as startup_target)
            upper_body_positions = self.startup_target
            ...
            self.get_logger().info(f"Published ia_robot_ik startup: lower_body=[{target_x}, {target_y}, {target_yaw:.3f}], upper_body={len(upper_body_positions)} joints")
            
            self.startup_complete = True
            self.get_logger().info("ia_robot_ik startup sequence complete.")
            
        except Exception as e:
            self.get_logger().error(f"Error during ia_robot_ik startup sequence: {e}")
            self.startup_complete = True  # Continue anyway
    ...
    def control_loop(self):
        """Main control loop"""
        # Skip control loop until startup sequence is complete
    ...
    def publish_sdf_markers(self):
        ...
        try:
            ...
            sample_step = max(1, self.sdf.grid_size[0] // ObstacleAvoidanceConfig.SDF_VISUALIZATION_SAMPLE_RATE)
            ...
            for i in range(0, self.sdf.grid_size[0], sample_step):
                for j in range(0, self.sdf.grid_size[1], sample_step):
                    for k in range(0, self.sdf.grid_size[2], sample_step):
                        ...
                        if distance < ObstacleAvoidanceConfig.SDF_VISUALIZATION_THRESHOLD:  
                            marker = Marker()
                            ...
                            if marker_id > ObstacleAvoidanceConfig.VISUALIZATION_MAX_MARKERS:
                                break
...
def main(args=None):
    ...
    try:
        ...
        print("")
        print("Usage:")
        print("  python3 mobile_ik_controller.py              # Use standard startup sequence")
        print("  python3 mobile_ik_controller.py --use-ik     # Use IK startup sequence")
        print("")
