...
"""
Configuration for mobile manipulator IK solver.
Adjust these parameters to balance between task completion, obstacle avoidance, and motion constraints.

Note: SDF_BOUNDS define the workspace relative to the robot's current position
(robot-centered bounds) rather than absolute world coordinates.
"""

import numpy as np

class MobileIKConfig:
    """Configuration parameters for mobile IK solver including obstacle avoidance"""
...
class PresetConfigs:
    ...
    def high_precision():
        ...
        MobileIKConfig.SDF_RESOLUTION = 0.05
        MobileIKConfig.POINTCLOUD_DOWNSAMPLE_SIZE = 5000
        MobileIKConfig.SDF_UPDATE_RATE = 2
    ...
    def performance_mode():
        ...
        MobileIKConfig.SDF_RESOLUTION = 0.12
        MobileIKConfig.POINTCLOUD_DOWNSAMPLE_SIZE = 1500
        MobileIKConfig.SDF_UPDATE_RATE = 8
        MobileIKConfig.OBSTACLE_AVOIDANCE_WEIGHT = 7.0
        MobileIKConfig.COLLISION_MARGIN = 0.12
        print("✓ Applied PERFORMANCE configuration")
    
    @staticmethod
    def aggressive_avoidance():
        """Very aggressive obstacle avoidance"""
        MobileIKConfig.OBSTACLE_AVOIDANCE_WEIGHT = 25.0
        MobileIKConfig.COLLISION_MARGIN = 0.25
        MobileIKConfig.COLLISION_THRESHOLD = 1.0
        MobileIKConfig.COLLISION_COST_EXPONENTIAL_RATE = 15.0
    ...
    def conservative_avoidance():
        ...
        MobileIKConfig.OBSTACLE_AVOIDANCE_WEIGHT = 3.0
        MobileIKConfig.COLLISION_MARGIN = 0.1
        MobileIKConfig.COLLISION_THRESHOLD = 0.5
        MobileIKConfig.COLLISION_COST_EXPONENTIAL_RATE = 5.0
