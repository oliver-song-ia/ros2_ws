...
class SimpleMobileIK:
    ...
    def inverse_kinematics(self, left_target, right_target, q_init=None, mode="rest"):
        ...
        def objective(q):
            ...
            symmetry_error = np.abs(q[arm0_left_idx] - q[arm0_right_idx]) * MobileIKConfig.SYMMETRY_WEIGHT
            ...
            regularization = MobileIKConfig.REGULARIZATION_WEIGHT * np.linalg.norm(q - q_init)
            ...
            chassis_xy_penalty = MobileIKConfig.CHASSIS_XY_PENALTY * (chassis_x_change**2 + chassis_y_change**2)
            chassis_z_penalty = MobileIKConfig.CHASSIS_Z_PENALTY * chassis_theta_change**2
            ...
            if mode in ('approach', 'rest', 'wait_for_arm_raise', 'retreat'):
                chest1_reward = MobileIKConfig.CHEST_PENALTY_APPROACHING * chest1_angle # max(0, (chest1_angle - (np.pi/8)))
            else:
                chest1_reward = MobileIKConfig.CHEST_PENALTY * chest1_angle
            # else:
            #     chest1_reward = 0.0
            ...
            arm1_reward = MobileIKConfig.ARM1_PENALTY * ((arm1_left_angle - (np.pi/2))**2 + (arm1_right_angle - (np.pi/2))**2)
            ...
            #         obstacle_cost = MobileIKConfig.OBSTACLE_AVOIDANCE_WEIGHT * collision_cost
            #     except Exception as e:
            #         self.logger.warning(f"Error computing collision cost: {e}")
            
            # Calculate weighted costs
            position_cost = MobileIKConfig.POSITION_WEIGHT * (left_pos_error + right_pos_error)
            orientation_cost = MobileIKConfig.ORIENTATION_WEIGHT * (left_rot_error + right_rot_error)
        ...
        for i, name in enumerate(self.joint_names):
            ...
            if name in ['chassis_x_joint', 'chassis_y_joint', 'chassis_rotation_joint']:
                max_step = MobileIKConfig.MAX_CHASSIS_STEP
            else:
                max_step = MobileIKConfig.MAX_JOINT_STEP
        ...
        def position_constraint(q):
            ...
            return MobileIKConfig.MAX_POSITION_ERROR - total_pos_error
        ...
        def chest1_constraint(q):
            ...
            return MobileIKConfig.CHEST_MAX_ANGLE - q[chest1_idx]
        ...
        try:
            ...
            if result.success:
                ...
                if max_change > MobileIKConfig.MAX_JOINT_STEP * 0.95:
