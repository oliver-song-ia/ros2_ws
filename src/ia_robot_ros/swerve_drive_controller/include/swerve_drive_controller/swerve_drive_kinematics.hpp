...
struct WheelCommand
{
  ...
  double drive_velocity;  // Drive angular velocity in radians per second
};
...
public:
  ...
  explicit SwerveDriveKinematics(const std::array<std::pair<double, double>, 4> & wheel_positions, 
                                 ...
                                 double min_steering_angle = -M_PI / 2.0);
  /**
   ...
   * @return Array of wheel commands (steering angles and drive angular velocities in rad/s).
   */
   ...
   * @param wheel_positions New wheel positions.
   * @param wheel_radius New wheel radius.
   * @param max_steering_angle New maximum steering angle.
   * @param min_steering_angle New minimum steering angle.
   */
...
private:
  ...
  double min_steering_angle_;                  // Minimum steering angle in radians
  ...
  /**
   * @brief Clip steering angle to the min/max range.
   * @param wheel_commands Array of wheel commands to be clipped.
   * @return if clipping was successful.
   */
  bool clip_steering_angle(std::array<WheelCommand, 4> & wheel_commands);
};
}  // namespace swerve_drive_controller
