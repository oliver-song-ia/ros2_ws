...
std::array<std::pair<double, double>, 4> wheel_positions_ = {{
  ...
  {0.5, 0.25227},   // front left
  {0.5, -0.25227},    // front right
  {-0.5, 0.25227},  // rear left
  {-0.5, -0.25227}    // rear right
}};

double wheel_radius_ = 0.085;
double max_steering_angle_ = M_PI / 2.0;
double min_steering_angle_ = -M_PI / 2.0;

SwerveController::SwerveController()
: controller_interface::ControllerInterface(), swerveDriveKinematics_(wheel_positions_, wheel_radius_, max_steering_angle_, min_steering_angle_)
{
...
CallbackReturn SwerveController::on_configure(const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  try
  {
    // Update the kinematics with the wheel parameters
    double wheel_x_offset = params_.wheel_x_offset;
    double wheel_y_offset = params_.wheel_y_offset;
    wheel_positions_ = {{
      // https://control.ros.org/rolling/doc/ros2_controllers/doc/mobile_robot_kinematics.html
      {wheel_x_offset, wheel_y_offset},   // front left
      {wheel_x_offset, -wheel_y_offset},    // front right
      {-wheel_x_offset, wheel_y_offset},  // rear left
      {-wheel_x_offset, -wheel_y_offset}    // rear right
    }};
    swerveDriveKinematics_.update_parameters(
      wheel_positions_, params_.wheel_radius, params_.max_steering_angle, params_.min_steering_angle);
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  // Disabled halt to allow odom calculation in open-loop mode
  ...
  // Find the maximum absolute drive velocity across all wheels
  double max_abs_velocity = 0.0;
  for (std::size_t i = 0; i < 4; ++i)
  {
    double abs_velocity = std::abs(wheel_command[i].drive_velocity);
  ...
  // If maximum velocity exceeds threshold, scale all wheel velocities
  if (max_abs_velocity > params_.velocity_threshold)
  {
    double scaling_factor = params_.velocity_threshold / max_abs_velocity;
    for (std::size_t i = 0; i < 4; ++i)
    {
      wheel_command[i].drive_velocity *= scaling_factor;
    }
  }

  // Apply steering angle error compensation to prevent wheel scrubbing
  // Get current steering angles and apply commands with velocity scaling
  double steering_error_velocity_scale = 1.0;
  for (std::size_t i = 0; i < 4; i++)
  {
    ...
    // Set the commanded steering angle
    axle_handles_[i]->set_position(wheel_command[i].steering_angle);

    // Only apply steering angle error compensation in closed-loop mode
    if (!params_.open_loop)
    {
      ...
      double steering_error = std::abs(std::atan2(
        std::sin(target_steering_angle - current_steering_angle),
        std::cos(target_steering_angle - current_steering_angle)
      ));
      
      // Scale wheel velocity based on steering angle error
      if (steering_error > params_.steering_angle_tolerance)
      {
        // Linear scaling between tolerance and max error
        double error_range = params_.max_steering_error_for_zero_velocity - params_.steering_angle_tolerance;
        if (error_range > 0.0)
        {
          _velocity_scale = std::max(0.0, 
            1.0 - (steering_error - params_.steering_angle_tolerance) / error_range);
          steering_error_velocity_scale = 
            std::min(steering_error_velocity_scale, _velocity_scale);
        }
        else
        {
          // If error_range is zero or negative, scaling cannot be performed.
          // Only stop the wheel if the steering error is greater than the tolerance.
          if (steering_error > params_.max_steering_error_for_zero_velocity)
          {
            steering_error_velocity_scale = 0.0;
          }
          // Optionally, log a warning about misconfiguration
          static bool warned = false;
          if (!warned)
          {
            RCLCPP_WARN(
              rclcpp::get_logger("swerve_drive_controller"),
              "max_steering_error_for_zero_velocity (%.3f) <= steering_angle_tolerance (%.3f). "
              "Velocity scaling is disabled; please check your parameters.",
              params_.max_steering_error_for_zero_velocity,
              params_.steering_angle_tolerance);
            warned = true;
          }
        }
      }
  ...
  for (std::size_t i = 0; i < 4; i++)
  {
    // Apply scaled velocity command to wheel
    double scaled_velocity = wheel_command[i].drive_velocity * steering_error_velocity_scale;
    wheel_handles_[i]->set_velocity(scaled_velocity);
  }
  ...
  // VALIDATED: data successfully passed from /joint_states
