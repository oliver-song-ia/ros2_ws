...
class JointPositionFilter:
    """Lightweight exponential moving average filter for streaming joint positions."""
    ...
    def apply(self, joints: np.ndarray) -> np.ndarray:
        """Smooth joints inplace by blending with previous estimate."""
        if joints is None:
            return joints

        if self._previous is None or not self.enabled:
            self._previous = joints.copy()
            return joints

        if joints.shape != self._previous.shape:
            # Reset if joint dimensionality changes unexpectedly.
            self._previous = joints.copy()
            return joints

        filtered = self.alpha * joints + (1.0 - self.alpha) * self._previous
...
class PoseDetector(Node):
    ...
    def __init__(self):
        ...
        # Declare parameters
        self.declare_parameter('config_path', f'{current_dir}/experiments/Custom/pretrained-full')
        self.declare_parameter('model_path', f'{current_dir}/experiments/Custom/pretrained-full/spike_oliver_1022.pth')
        ...
        self.joint_filter_alpha = self.get_parameter('joint_filter_alpha').get_parameter_value().double_value
        ...
        # Initialize joint smoothing filter
        self.joint_filter = JointPositionFilter(alpha=self.joint_filter_alpha)
        ...
        self.get_logger().info(f'Subscribing to: /pointcloud_human_only')
        ...
        try:
            ...
            if result[0] is not None:
                ...
                if np.any(~np.isfinite(joints)):
                    self.get_logger().warn('Non-finite joint prediction detected, resetting filter')
                    self.joint_filter.reset()
                    return

                joints = self.joint_filter.apply(joints)
