#include "turn_on_ia_robot/wheeltec_tcp_robot.h"
#include "turn_on_ia_robot/Quaternion_Solution.h"
#include "wheeltec_robot_msg/msg/data.hpp" 
#include "turn_on_ia_robot/crc.h"
...
void turn_on_robot::func1Thread(){
  ...
  Cmd_Vel_New = create_subscription<stm_msg::msg::Tcpdata>(
      "cmd_vel_new", 2, std::bind(&turn_on_robot::Cmd_Vel_Callback, this, _1));

  // Cmd_Motor_Status = create_subscription<sensor_msgs::msg::JointState>(
  //     "robot/joint_command", 2, std::bind(&turn_on_robot::Ia_Joint_Commands_Callback_Status_New, this, _1));
  ...
  Ia_Motor_Status = create_subscription<sensor_msgs::msg::JointState>(
       "joint_commands", 2, std::bind(&turn_on_robot::Ia_Joint_Commands_Callback_Status_New, this, _1));
...
//新话题接收
void turn_on_robot::Cmd_Vel_Callback(const stm_msg::msg::Tcpdata::SharedPtr twist_aux){
...
void turn_on_robot::Ia_Joint_Commands_Callback_Status_New(const sensor_msgs::msg::JointState::SharedPtr staus){
  ...
  #ifndef IA_NEW_ROBOT
  for(int i=0;i<MOTOR_JOINT_LEN;i++){
...
void turn_on_robot::Ia_Joint_Commands_Callback_Status(const sensor_msgs::msg::JointState::SharedPtr staus){
  ...
  //float round = PI*STM_DIAM/1000; //改为m
  float round = PI*2;
  float speed = WHEEL_FRONT_LEFT*(staus->velocity[1])*60/round;  //转换为m/s
  speed_data = static_cast<int>(speed);
  send_data[6] = 0x01;    //使能
  send_data[7] = 0x01;
  send_data[8] = (uint8_t)(speed_data/256);
  send_data[9] = (uint8_t)(speed_data);
  send_data[10] = 0x64;   //加速度
  speed = WHEEL_FRONT_RIGHT*(staus->velocity[0])*60/round;  //转换为m/s
  speed_data = static_cast<int>(speed);
  send_data[11] = 0x01;    //使能
  send_data[12] = 0x01;
  send_data[13] = (uint8_t)(speed_data/256);
...
void turn_on_robot::Robot_Data_Deal(char* data,int len){
  ...
  if(data[1]==CMD_TYPE_JOINT_MOTOR){
    ...
    if(data[6]==0x11){
      ...
      robot_joint_status.velocity[17] = WHEEL_REAR_LEFT*Odom_Trans(data[9],data[10])*160.0/60.0*STM_PI;
      robot_joint_status.position[17] = 0;
      
      joint_publish_flag |=0x30000;
    }else{
      robot_joint_status.velocity[16] = WHEEL_FRONT_RIGHT*Odom_Trans(data[11],data[12])*160.0/60.0*STM_PI;
      robot_joint_status.position[16] = 0;
      robot_joint_status.velocity[18] = WHEEL_REAR_RIGTH*Odom_Trans(data[11],data[12])*160.0/60.0*STM_PI;
...
void turn_on_robot::Ia_Write_Log_To_File(char* buf,int len){
  ...
  for(int i=0;i<len;i++){
    snprintf(data,sizeof(data),"%02X",data);
    //printf(" %x",buf[i]);
    fprintf(ia_fp, data);
  }
